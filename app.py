...
with st.expander("📘 首次使用？点击查看使用教程", expanded=False):
    st.markdown("""
### 1. 工具运行流程

下载模板 → 填写各个 Sheet → 上传 Excel → 输入 API Key → 设置每批分析数量 → AI 自动分类评论 → 生成分类结果 → 自动生成数据分析 Dashboard → 下载结果 Excel。
...
- 存放需要 AI 分类的评论数据。
- 必填字段：【买家备注】。
- 建议保留：【亚马逊SKU】、【订单号】、【退货时间】，用于结果回填、问题定位和月度趋势分析。

**销量**
- 用于结合 SKU 和月份计算【问题率】。
- 需要字段：【亚马逊SKU】、【月份】、【销量】。
- 如果销量数据缺失，系统仍可完成 AI 分类，但问题率热力图、趋势图、气泡图等分析会受影响。

**层级表**
- 用于定义 AI 的分类体系。
- AI 会优先按照这里的分类框架判断评论属于哪个问题类型。
- 常见字段包括：【一级场景】、【二级维度】、【三级对象】、【四级问题】、【五级描述】。

**产品描述**
- 用于告诉 AI 这个产品是什么。
- 建议填写产品名称、功能、使用场景、适用人群、材质、尺寸、卖点等信息。

**分类案例**
- 用于放人工分类示例。
- AI 会参考这些案例，提高分类准确率和结果一致性。
...
- 评论为空、NA、N/A、None、null、- 等内容不会调用 AI。
- 历史已分析评论不会重复调用 AI，只会分析新增评论。
- 【退货时间】建议填写完整日期，例如 2026-01-15。
- 【月份】建议填写 yyyy-MM 格式，例如 2026-01。
- API Key 仅用于本次分析，不会写入导出文件。
    """)
